...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom name to use for output files",
    )
...
def process_collection_and_search(filtered_queries, args, model):
    ...
    results_path = os.path.join(
        args.output_dir,
        f"{os.path.basename(args.output_name)}_inference_run_results.tsv",
    )
    final_df.to_csv(results_path, sep="\t", index=False)

    # Save timing information
    timing_path = os.path.join(
        args.output_dir, f"{os.path.basename(args.output_name)}_inference_timing.json"
    )
